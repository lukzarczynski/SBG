...
public class WithOneEnemyPiece extends Operator {
    ...
    public Predicate<OneMove> matches() {
        return move -> {
            ...
            for (int i = 0; i < move.getMoves().size() - 1; i++) {
                ...
                if (move.getMoves().get(i).getMoveType().equals(MoveType.PIECE)) {
                    enemyPieces++;
                }
            }
        ...
        return om -> {
            ...
            for (int i = 0; i < om.getMoves().size() - 1; i++) {
                if (om.getMoves().get(i).getMoveType().equals(MoveType.EMPTY)) {
                    ...
                    copy.getMoves().get(i).setMoveType(MoveType.PIECE);
                    result.add(copy);
                }
    ...
    public String getDescription() {
        return "With one enemy piece on the way";
    }
}
